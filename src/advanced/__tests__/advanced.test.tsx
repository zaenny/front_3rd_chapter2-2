...
import { checkIsOutOfStock } from "../../refactoring/hooks/utils/checkIsOutOfStock";
import { useCartCount } from "../../refactoring/hooks/useCartCount";
...
describe("advanced > ", () => {
  ...
  describe("자유롭게 작성해보세요.", () => {
    test("새로운 유틸 함수를 만든 후에 테스트 코드를 작성해서 실행해보세요", () => {
      ...
      expect(checkIsOutOfStock(inStockProduct)).toBe(false);
      expect(checkIsOutOfStock(outOfStockProduct)).toBe(true);
    });

    test("새로운 hook 함수르 만든 후에 테스트 코드를 작성해서 실행해보세요", () => {
      const testCart: CartItem[] = [
        {
          product: { id: "1", name: "A", price: 1000, stock: 5, discounts: [] },
          quantity: 2,
        },
        {
          product: { id: "2", name: "B", price: 2000, stock: 3, discounts: [] },
          quantity: 1,
        },
      ];

      const { result } = renderHook(() => useCartCount(testCart));
